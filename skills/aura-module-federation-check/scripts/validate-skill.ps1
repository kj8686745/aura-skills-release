...
if (Test-Path -LiteralPath $skillFile) {
	...
	if ($skillContent -notmatch '(?m)^name:\s*aura-module-federation-check\s*$') { [void]$errors.Add('SKILL.md name 不正确。') }
	...
	foreach ($keyword in @('aura-web-develop', 'provider-consumer-handoff.md', '改前', '复检', '对外接入说明', '首次调用提示')) {
...
if (Test-Path -LiteralPath $usagePath) {
	...
	foreach ($keyword in @('$aura-module-federation-check', '示例提示词')) {
...
if (Test-Path -LiteralPath $openAiPath) {
	...
	if (-not $openAiContent.Contains('$aura-module-federation-check')) { [void]$errors.Add('agents/openai.yaml 默认提示未显式引用技能名。') }
}
...
Write-Host "✓ aura-module-federation-check $version 技能校验通过。" -ForegroundColor Green
