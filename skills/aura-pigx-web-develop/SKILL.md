...
准备调用任何其它技能前，必须先检查当前环境的可用技能列表，不得仅凭目录名或历史安装记录假定存在。规则适用于 `$frontend-design`、`fmap-2d`、`fxft-video`、`aura-pigx-module-federation-check`、`vueuse-functions` 以及后续参考资料新增的任何外部技能。
...
8. **适配模块联邦**：涉及 remote、expose、manifest、远程菜单、shared、运行时入口或模块联邦配置时，先检查 `aura-pigx-module-federation-check` 是否可用；缺失时提示用户安装。可用后先建立基线，由本技能完成实现后再次复检。远程页面不得依赖提供方 `main.ts` 的全局注册副作用。
