...
Write-Host "=== aura-web-develop 技能校验 ===" -ForegroundColor Cyan
...
if (Test-Path -LiteralPath $skillFile) {
  ...
  if ($skillContent -notmatch "(?m)^name:\s*aura-web-develop\s*$") {
...
if (Test-Path -LiteralPath $usageFile) {
  ...
  foreach ($keyword in @('$aura-web-develop', '示例提示词', '@fxft/ui-plus')) {
...
if ((Test-Path -LiteralPath $openAiFile) -and -not ((Get-Content -LiteralPath $openAiFile -Raw -Encoding UTF8).Contains('$aura-web-develop'))) {
