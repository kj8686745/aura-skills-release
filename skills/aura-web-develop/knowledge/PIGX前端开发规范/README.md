...
- 模块联邦远程页面中的按需注册、自动解析或自动导入组件由提供方优先通过编译期 Resolver 注入组件代码与配套样式；宿主不承担提供方组件库的安装或解析职责。
- 不能仅凭 `components.d.ts` 或运行时全局注册判断组件可用；Resolver 必须核验构建产物的本地 import，并完成独立运行、远程加载和控制台 `Failed to resolve component` 检查。
- 自动解析在 expose 场景失效时，优先修复项目级 Resolver、Windows 路径和 Resolver 顺序；确实无法覆盖时才显式导入组件及配套样式。
