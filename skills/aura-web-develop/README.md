...
`1.2.17` 补充模块联邦远程页面按需/自动解析组件的构建边界：提供方优先使用编译期 Resolver 注入组件与样式，宿主零额外组件库依赖，并核验构建产物及独立、远程两种运行方式。
...
3. 模块联邦配置由本技能主导实现，并在修改前后调用 `aura-module-federation-check` 做基线和复检。
